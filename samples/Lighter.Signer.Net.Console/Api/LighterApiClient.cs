...
public sealed class LighterApiClient : ILighterApiClient, IAsyncDisposable
{
    ...
    {
        AccountResponse response = await GetAsync<AccountResponse>($"api/v1/account?by=index&value={accountIndex}", authToken: null, cancellationToken);
    ...
    {
        NonceResponse response = await GetAsync<NonceResponse>($"api/v1/nextNonce?account_index={accountIndex}&api_key_index={apiKeyIndex}", authToken: null, cancellationToken);
        return response.Nonce;
    }

    public async Task<MarketDetails> GetMarketAsync(string symbol, CancellationToken cancellationToken)
    {
        MarketsResponse response = await GetAsync<MarketsResponse>("api/v1/orderBookDetails?filter=perp", authToken: null, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        using HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Post, "api/v1/sendTx") { Content = form };
        ...
        where T : ApiResponse
    {
        ...
        catch (JsonException exception)
        {
            if (!response.IsSuccessStatusCode)
                throw new LighterApiException($"Exchange request failed with HTTP status {(int)response.StatusCode}.", exception);
        ...
        if (!response.IsSuccessStatusCode)
        {
            string detail = string.IsNullOrWhiteSpace(result?.Message)
                ? string.Empty
                : $": {result.Message.Trim()}";
            throw new LighterApiException($"Exchange request failed with HTTP status {(int)response.StatusCode}{detail}");
        }
